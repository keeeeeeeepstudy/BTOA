...
class TSEncoder(nn.Module):
    ...
    def forward(self, x, mask=None):  # x: B x T x input_dims
            ...
            x[~nan_mask] = 0
            x = self.input_fc(x.to(dtype=torch.float32))  # B x T x Ch
...
class MLPBlock(nn.Module):
    def __init__(self, Length, gamma=0.9):
        super().__init__()
        self.MLP = SamePadConv(Length, gamma=gamma)
    
    def ctrl_params(self):  
        c_iter = chain(self.MLP.controller.parameters(), self.MLP.calib_w.parameters(), 
                self.MLP.calib_b.parameters(), self.MLP.calib_f.parameters(),
                )
