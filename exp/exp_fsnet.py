...
class Exp_TS2VecSupervised(Exp_Basic):
    ...
    def calculate_similarity_latents(self, sample):
        vae = self.load_vae()
        qz_params = vae.encoder.forward(sample.to(self.device).float()).view(sample.size(0), self.args.latent_dim, vae.q_dist.nparams).data
    ...
    def train(self, setting):
        ...
        if not os.path.isfile(self.args.save+'/checkpt-0000.pth'):
            vae_model = train_VAE(train_loader, self.args, 'cuda:0')
    ...
    def get_adjust_data(self, x, y):
        buff_x, buff_y, logits = self.buffer.get_data(99) #[1,60,14].[1,1,7]
        x_aug=torch.cat((x[:,:,:7],buff_x[:,:,:7]),dim=0)

        similarities = self.calculate_similarity_latents(x_aug)
        #print(similarities)
        out, inds = torch.topk(similarities[0],15)
        print(out,inds)
        #out, inds = torch.max(similarities,dim=1)#查找每一个样本与其相似度最大的一个样本
        x_aug=torch.cat((x[:,:,:7],x[inds+1]),dim=0)
        aug_sample1 = gen_aug(x_aug, self.args.aug1)
        aug_sample2 = gen_new_aug_2(gen_aug(x_aug, self.args.aug2), self.args, inds, out, similarities)
        return 
        #aug_sample2 = gen_new_aug_2(gen_aug(sample, self.args.aug2), self.args, inds, out, self.device, similarities)
    ...
    def _ol_one_batch(self,dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        for _ in range(self.n_inner):
            if self.args.use_amp:
                ...
                if self.buffer.len()<100:
                    ...
                    self.get_adjust_data(x,batch_y)
                    buff_y = rearrange(buff_y, 'b t d -> b (t d)').float().to(self.device)
                    true = torch.cat((true,buff_y),dim=0)

                    outputs = self.model(x)

                    #print(buff_x.shape,buff_y.shape,x1.shape,outputs.shape,true.shape)
        ...
        outputs=outputs[:1,:]
        f_dim = -1 if self.args.features=='MS' else 0
        batch_y = batch_y[:1,-self.args.pred_len:,f_dim:].to(self.device)
